...
with st.sidebar:
    ...
    kerf_1d = st.number_input("Rzaz piły [mm]", value=4.5)
    trim_1d = st.number_input("Naddatek obcięcia [mm]", value=40.0)
    price_per_kg = st.number_input("Cena stali [PLN/kg]", value=4.60)

st.title("🏗️ SteelOpt: Optymalizator i Stykowanie")

uploaded_file = st.file_uploader("Wczytaj BOM (Excel/CSV):")
...
if uploaded_file:
    ...
    with tab1:
        ...
        st.download_button("📥 Pobierz Zestawienie w Excelu (2 Opcje)", data=excel_data, file_name="Zestawienie_SteelOpt.xlsx")

    with tab2:
        st.markdown("### Szablon zapytania ofertowego")
        st.info("Skopiuj poniższy tekst i wyślij do dostawcy. Zgodnie z wytycznymi z widoku usunięto tabelę.")
